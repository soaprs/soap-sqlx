...
- Updated `soaprs-core`, `soaprs-repository`, and shared contract-test
  dependencies from `0.1` to `0.3`. The adapter implementation remains source
  compatible while its public repository traits now have the same crate
  identity as applications using `soaprs 0.3`.
